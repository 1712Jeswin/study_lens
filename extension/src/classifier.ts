...
export function classifySite(url: string): SiteCategory {
  ...
  const checkCategory = (list: string[]) => 
    list.some((site) => domain === site || domain.endsWith(`.${site}`))

  // Check domain against each list in order: ai → coding → study → distraction
  if (checkCategory(SITE_LISTS.ai)) return 'ai'
  if (checkCategory(SITE_LISTS.coding)) return 'coding'
  if (checkCategory(SITE_LISTS.study)) return 'study'
  if (checkCategory(SITE_LISTS.distraction)) return 'distraction'
 ...
 */
export function isProductiveCategory(category: SiteCategory): boolean {
