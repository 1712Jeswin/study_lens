 ...
 */
function renderReflectionPrompt(entry: ReflectionEntry | null): void {
  ...
  if (!section || !messageEl || !btnYes || !btnNo) return
 ...
 */
async function handleReflection(solvedByAI: boolean): Promise<void> {
  try {
 ...
 */
function renderScoreHeader(score: DependencyScore): void {
  const valueEl = document.getElementById('score-value')
  const labelEl = document.getElementById('score-label')
  const dateEl = document.getElementById('score-date')
  const ringFill = document.getElementById('ring-fill')

  if (!valueEl || !labelEl || !dateEl || !ringFill) return
 ...
 */
function renderInsight(insight: DailyInsight): void {
  ...
  if (!section || !headlineEl || !warningEl || !suggestionEl) return
 ...
 */
function renderMetrics(insight: DailyInsight): void {
  ...
  if (!switchesEl || !deepworkEl || !practiceEl) return
 ...
 */
function buildAITypeLabel(record: DailyRecord): string {
 ...
 * @returns A formatted type label string
 */
function buildCodingTypeLabel(record: DailyRecord): string {
  const practice = record.practiceSeconds ?? 0
  const contest = record.contestSeconds ?? 0
  if (practice === 0 && contest === 0) return 'Reading'
  if (contest > 0) return `Contest: ${formatSeconds(contest)}`
 ...
 * @returns A formatted type label string
 */
function buildStudyTypeLabel(record: DailyRecord): string {
  const video = record.videoSeconds ?? 0
  if (video > 0) return `Video: ${formatSeconds(video)}`
  return 'Reading'
}
 ...
 * @param record - Today's DailyRecord
 */
function renderBreakdownTable(record: DailyRecord): void {
  ...
  const barWidth = (seconds: number): string =>
    totalAll === 0 ? '0%' : Math.round((seconds / totalAll) * 100) + '%'
 ...
 */
function renderTopics(tags: TopicTag[]): void {
 ...
 */
function renderStreak(streak: StreakData): void {
