 ...
 */
export function calculateDependencyScore(record: DailyRecord): DependencyScore {
  ...
  let label: DependencyScore['label'] = 'healthy'
 ...
 */
export function getScoreColour(label: DependencyScore['label']): string {
  switch (label) {
    case 'healthy':
      return '#1D9E75' // green
    case 'moderate':
