...
export const DEFAULT_DAILY_RECORD = (date: string = getTodayISO()): DailyRecord => ({
  date,
  ai: 0,
  coding: 0,
  study: 0,
 ...
 */
export async function updateTodayRecord(category: SiteCategory, secondsToAdd: number): Promise<void> {
  const today = getTodayISO()
  try {
 ...
 */
export async function getPendingReflection(): Promise<ReflectionEntry | null> {
  try {
 ...
 * @returns Promise that resolves when the write is complete
 */
export async function setPendingReflection(entry: ReflectionEntry | null): Promise<void> {
  try {
 ...
 * @returns Promise that resolves when the write is complete
 */
export async function addReflectionToHistory(entry: ReflectionEntry): Promise<void> {
  try {
    const data = await getStorageData()
 ...
 * @returns Promise that resolves when the write is complete
 */
export async function updateActivitySeconds(field: keyof DailyRecord, secondsToAdd: number): Promise<void> {
  try {
    const data = await getStorageData()
    const today = getTodayISO()
    const records = data.records
    ...
    const current = (record[field] as number | undefined) ?? 0
 ...
 * @returns Promise that resolves when the write is complete
 */
export async function updateTopicTag(name: string, seconds: number, isPractice: boolean): Promise<void> {
  try {
    const data = await getStorageData()
    const today = getTodayISO()
    const records = data.records
    ...
    const topicTags: TopicTag[] = record.topicTags ?? []
    const existing = topicTags.find(t => t.name === name)

    if (existing) {
      existing.totalSeconds += seconds
      if (isPractice) existing.practiceSeconds += seconds
    } else {
      topicTags.push({
        name,
 ...
 */
export async function getSessionLog(): Promise<SessionEntry[]> {
  try {
